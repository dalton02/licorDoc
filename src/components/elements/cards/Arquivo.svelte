...
<div class="flex flex-col justify-start items-start p-2 lg:p-4 gap-1 lg:gap-3  overflow-hidden">
    ...
    {#if isOpen}
        ...
        {#each matchs as  match} 
          <Button className="flex gap-2 lg:gap-4 items-center justify-start hover:bg-lightStrong hover:dark:bg-darkWeak w-full overflow-hidden rounded-lg p-1 px-3 lg:px-6"
          onClick={()=>go(match)}>
            {#if match.type==="code"}
